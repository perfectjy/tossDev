;(function(win, doc, undefined) {
	...
	KAUI.common = {
		...
		reitsEffect : {
			...
			slide : function () {
				...
				$reitsSlide.on("afterChange", function (event, slick, currentSlide) {                    
					$('.reits-item').removeAttr('tabindex');
					$("#slider-page").find('.current').text(parseInt(slick.currentSlide + 1));
					$("#slider-page").find('.total').text(slick.slideCount);
		...
		// 	if(!$('.ui-tab.link').length) return;
		
		// 	var gnbH = $('.gnb').height();
		// 	var gId = $('.ui-tab.link').attr('id');
		// 	var el = $('#' + gId);			
		// 	var	elTop = el.offset().top - gnbH;	
		// 	var tabH = el.find('.ui-tab-wrap').outerHeight();
		// 	var $menu = el.find('.ui-tab-list li');	
		// 	var $contBox = el.find('.ui-tab-pnl');			
		...
		// 		const contents = $contBox.eq(idx);
		// 		const contentsD = Math.ceil(contents.offset().top);			
		// 		const targetD = contentsD - tabH - gnbH;

		// 		console.log(contentsD + ',' + tabH + ',' + gnbH + ',' + idx);
		// 		$menu.removeClass('selected');
		// 		$menu.eq(idx).addClass('selected');
		// 		setTimeout(function() {
		// 			$('html,body').stop().animate({scrollTop:targetD});
		// 		},50);
				
		// 	});

		// 	$WIN.on('scroll', function () {		
		// 		($(this).scrollTop() >= elTop) ? el.addClass('fixed') : el.removeClass('fixed');
				
		// 		$contBox.each(function(){					
		// 			const targetTop = Math.ceil($(this).offset().top);
		// 			const targetD = targetTop - tabH - gnbH;	

		// 			if($WIN.scrollTop() >= targetD ){
		// 				const idx = $(this).index();
...
var mainVisualSlide = function () {
	...
	$('.main-visual .slick-control .pause').on('click',function(){		
		...
		$('.main-visual .slick-control .play').show().focus();
		$(this).hide();
